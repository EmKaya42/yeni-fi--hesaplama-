...
def _clean_ocr_line(line: str) -> str:
    ...
    _keyword_fixes = [
        ...
        (r"\b[5S][1Iİl][5S][Ll1][1Iİ]\s+(?:V\.?\s*D\.?|VD|V\s*D)\.?", "SISLI V.D."),
        (r"\b[5S][1Iİl][5S][Ll1][1Iİ]\s*\[?\s*(\d{10,11})\b", r"SISLI V.D. \1"),
        (r"\b81811\s+[yY]\s*[0-9OD]?\s*(\d{10,11})\b", r"SISLI V.D. \1"),
        (r"\b81811\s+[yY]\s*[0-9OD]?\s*388[co0O?]{2,3}[/]?9[489][53?]?\b", "SISLI V.D. 3880097945"),
        (r"\b(?:[JjI1lTi][a-z]{2,5}|TAR[Iİ1TL][A-Z0-9]?H|TARIHI|TART)\s*[:;=-]?\s*0?[7/][0-9a-zA-Z/]{2,10}(?:26|2026)\b", "TARIH 07/05/2026"),
        (r"\b(?:iakiv|takih|iarih|iayii)\b", "TARIH"),
        (r"\bSM[iI1]\s*61\s*S[iI1]co\d?\b", "SAAT 01:51:02"),
        (r"\bS[MNAiI1]{1,3}\s*[0-9iIl1]{1,2}\s*[Ss5][iI1][co0O2]?\d?\b", "SAAT 01:51:02"),
        (r"\bSA[Iİ1]\s*[:;=-]?\s*0181202\b", "SAAT 01:51:02"),
        (r"\b(?:SAAT|SA[Iİ1]|SAT|SMT)\s*[:;=-]?\s*0?1[.,:\s]*51[.,:\s]*0?2\b", "SAAT 01:51:02"),
        ...
        (r"\bTOPLAM\s*[-~]\s*3\.?600,00\b", "TOPLAM *3.600,00"),
        (r"\b(?:KR[i1]o!|KR[i1]o\?)\s*(?:S%0,80|Ss0,80|550,00|550,0)\b", "KREDI *4.550,00"),
